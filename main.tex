...
Assume that a reciprocal lattice vector, \gvec, satisfies a bragg condition.  The unit vector aligned with its associated diffracted beam, \dhat, is
\begin{align}
...
\end{align}
and \bhat\ is the unit vector aligned with the beam propgation direction.  \eqnref{E:braggCondition} is the brag condition for \xrays\ having wavelength $\lambda$, and \eqnref{E:intersectsDetector} is the geometric condition ensuring that \dhat\ can intersect the detector plane.  Note that there is a tertiary condition that the point \Pfour\ must also lie within the physical extent of the detector element, but that is an easily filetered result.

\begin{align}
\labcomps{\gvec} &= \rmats \samcomps{\gvec} \\
                 &= \rmats \defgrad^{-T} \xtlcomps{\gvec} \\
                 &= \rmats \defgrad^{-T} \bmat \begin{Bmatrix}h\\k\\l\end{Bmatrix} \\
                 &= \rmats \vmat^{-1} \rmatc^{T} \bmat \begin{Bmatrix}h\\k\\l\end{Bmatrix}
\end{align}
