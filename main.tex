...
\end{equation}
where $\labcomps{\dhat}$ is obtained from \eqnref{E:dhat} and \eqnref{E:gvec_l}.
% as
%\begin{align}
...
\begin{align}
    a \siw + b \cow &= c \label{E:trig}\\
                    ...
                    \mbox{where } R\equiv\sqrt{a^2 + b^2} &\mbox{, } \alpha\equiv\mathrm{atan2}(b, a),
\end{align}
and 
%%
\begin{align}
...
\end{equation}
which are either unique (both sides of the diffraction cone), a double root (tangent to the diffraction cone) or do not exist ($|\frac{c}{\sqrt{a^2 + b^2}}| > 1$, can't intersect diffraction cone).

\section{Summary}\label{S:summary}
The complete procedure for finding the detector coordinates of all reflections for a specifically oriented, strained crystal athe the position \Pthree\ consists of the following procedure:
\begin{enumerate}
