...
\figref{F:diffraction_schema}.  For generality we utilize four
...
\end{displaymath}
(see \secref{S:detector}, \ref{S:crystal}).  For the sake of convenience, the beam fram is defined such that $-\bhat \| \Zb$.  To define an azimuthal angle, $\eta$, an axis normal to the beam direction must be specified to define $\eta=0$.  This axis is represented by the unit vector \ehat.  The basis vectors defining \beamframe\ are therefore 
\begin{equation}
...
\end{equation}
In the beam frame, the expression for a reciprocal lattice vector, \gvec, is simply
\begin{equation}
  \gvec = \frac{2\sin{\theta}}{\lambda}\begin{Bmatrix}
    \cos\eta \cos\theta \\
    \sin\eta \cos\theta \\
    \sin\theta
  \end{Bmatrix}
