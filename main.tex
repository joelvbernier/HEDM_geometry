...
non-negligible physical extent in the lab frame, typically possessing a
...
direction as the unit incident wavevector, $\bhat\equiv\frac{\mbm{k}_i}{\|\mbm{k}_i\|}$.  
The centroid of the beam cross-section is defined to coincide with the origin
of \labframe, labeled \Pzero.

\subsection{The Beam Frame}
When working with the geometry of diffraction, it becomes very useful
to associate detector-relative cartesian coordinates -- and diffraction vectors \dhat\ -- with a bragg angle, $2\theta$, and
...
\end{equation}
where \dhat\ is the unit vector representing the direction of the diffracted beam
\begin{displaymath}
...
$\labcomps{\ehat}=\left[1\;0\;0\right]^T$.  The components \bhat\ in the
laboratory frame are parameterized by spherical angles $(\phi, \gamma)$:
\begin{equation}
